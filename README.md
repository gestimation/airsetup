...
confirmation, M11 SEMANTIC QC, SAP or analysis-plan drafting,
...
- `QC_SKILL_PLAN.md`: checks whether a general coding plan or non-SAP analysis
  specification is clear enough for R implementation.

- `QC_SKILL_SAP.md`: performs evidence-first clinical-trial SAP QC using
  a paraphrased 55-item SAP checklist and an M11/E9(R1)-informed supplement.
  It can use `M11SEMANTIC_MAP.md` or the supplied protocol when available, but
  can also review the SAP alone. Unavailable cross-document evidence is
  reported as `Cannot assess`, not completed by assumption.
...
- `QC_SKILL_M11SEMANTIC.md`: supports evidence-first M11-informed semantic
  organization across multiple clinical-trial materials before R planning or
  R coding. It uses a compact analysis-critical map and should be used only
  for M11/electronic-protocol tasks or complex clinical-trial semantic review,
  not ordinary context QC.
...
tracks. Across the clinical skills, the governing rule is: no evidence, no
study-specific assertion.
