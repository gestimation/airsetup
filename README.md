...
| `airsetup()` in `split` mode | `r_project/ai_hidden_data/` and `r_project/ai_hidden_data/initial_YYYYMMDD/` | Analysis data area that AI must not inspect. |
| `airsetup()` in `split` mode | `r_project/.gitignore` | Defaults that avoid committing hidden analysis data. |
| `airsetup()` in `split` mode | `r_project/README_DO_NOT_SHARE_WITH_AI.md` | Local reminder that the folder is for human/R execution. |
| `airskill()` | `ai_project/source/skills/SKILLS_INDEX.md` | Index and selection guide for QC skill files. |
| `airskill()` | `ai_project/source/skills/QC_SKILL_CONTEXT.md` | Lightweight context QC skill. |
| `airskill()` | `ai_project/source/skills/QC_SKILL_PLAN.md` | Plan and SAP implementability QC skill. |
| `airskill()` | `ai_project/source/skills/QC_SKILL_RESULT.md` | Result consistency and interpretation QC skill. |
| `airskill()` | `ai_project/source/skills/QC_SKILL_M11SEMANTIC.md` | M11-informed semantic context QC skill for complex clinical-trial materials. |
