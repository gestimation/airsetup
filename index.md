...
optional lightweight QC skill templates for context review, general plan
review, clinical SAP review, result review, and M11-informed semantic
review, and optional independent QC agent scaffolding.
...
uses `split = TRUE`, `skills = TRUE`, and `qc_agent = FALSE`. With
`split = TRUE`, it creates two sibling folders:
...
| [`airsetup()`](https://gestimation.github.io/airsetup/reference/airsetup.md) | `ai_project/agent_control/` | AI/agent control files, agent role definitions, and QC skill templates. |
| [`airsetup()`](https://gestimation.github.io/airsetup/reference/airsetup.md) | `ai_project/agent_control/AGENT_CONTROL_INDEX.md` | Index and selection guide for agent control files. |
...
| [`airskill()`](https://gestimation.github.io/airsetup/reference/airskill.md) | `ai_project/agent_control/QC_SKILL_CONTEXT.md` | Lightweight context QC skill. |
...
`ai_project/ai_visible_data/initial_YYYYMMDD/`, while a full analysis
data set with the same file name and the same data structure may be
placed in `r_project/ai_hidden_data/initial_YYYYMMDD/`.
...
- `QC_SKILL_PLAN.md`: checks whether a general coding plan or non-SAP
  analysis specification is clear enough for R implementation.

- `QC_SKILL_SAP.md`: performs evidence-first clinical-trial SAP QC using
  a paraphrased 55-item SAP checklist and an M11/E9(R1)-informed
  supplement. It can use `M11SEMANTIC_MAP.md` or the supplied protocol
  when available, but can also review the SAP alone. Unavailable
  cross-document evidence is reported as `Cannot assess`, not completed
  by assumption.
...
- `QC_SKILL_M11SEMANTIC.md`: supports evidence-first M11-informed
  semantic organization across multiple clinical-trial materials before
  R planning or R coding. It uses a compact analysis-critical map and
  should be used only for M11/electronic-protocol tasks or complex
  clinical-trial semantic review, not ordinary context QC.

- `AGENT_CONTROL_INDEX.md`: explains the agent control files and
  available QC skills.

`QC_SKILL_PLAN.md` and `QC_SKILL_SAP.md` are normally alternative review
tracks. Across the clinical skills, the governing rule is: no evidence,
no study-specific assertion.
...
Use
[`aircheck()`](https://gestimation.github.io/airsetup/reference/aircheck.md)
to verify that the required folders and files exist.
...
[`aircheck()`](https://gestimation.github.io/airsetup/reference/aircheck.md)
returns a data frame with one row per required item. It reports whether
each folder or file was found.
