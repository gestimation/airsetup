...
airsetupパッケージは、AIエージェントが支援するRワークフローのためのフォルダ構造を生成します。統計解析のための関数は提供されませんが、その代わりフォルダ、最小限のAGENTS.md、QC_STATUS.mdトラッカー、そしてオプションとして軽快なQCスキルテンプレートが用意されています。

QCの方法は1つではありません。あるプロジェクトではダブルプログラミングを行うかもしれませんし、別のプロジェクトでは目視チェックを行うかもしれません。AI
&
...
- このデモでは、ダミーデータとして、`cifmodeling`に含まれているデータフレームから抽出した、最初の3行を用いる
