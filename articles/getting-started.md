...
[`airsetup_demo()`](https://gestimation.github.io/airsetup/reference/airsetup_demo.md),
optional QC skill templates are generated in the `agent_control/`
...
- `QC_SKILL_PLAN.md`: checks whether a general coding plan or analysis
  specification contains enough information for R implementation.
- `QC_SKILL_SAP.md`: performs evidence-first clinical-trial SAP QC using
  ...
  supplement. An M11 semantic map can be used when available but is not
  required.
...
- `QC_SKILL_M11SEMANTIC.md`: supports evidence-first M11-informed
  ...
  R planning or R coding. It is based on a compact analysis-critical
  map. It should be used only for M11/electronic-protocol tasks or
  complex clinical-trial semantic review, not ordinary context QC.
